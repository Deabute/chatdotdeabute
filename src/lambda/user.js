...
const login = {
    ...
    compareHash: (event, user, success, issue) => {
        bcrypt.compare(event.body.password, user.passhash, (compareError, result) => {
            if (result) { success() }
    ...
    lambda: (event, context, callback) => {
        ...
        if (event.body) {
            mongo.connect((db, client) => {
                ...
                login.findUser(event, db, (userDoc) => {
                    login.compareHash(event, userDoc, () => {
                        login.assignToken(
                            event,
                            userDoc,
                            db,
    ...
    lambda: (event, context, callback) => {
        ...
        if (event.body && event.body.password && event.body.username) {
            mongo.connect((db, client) => {
                ...
                signup.checkExisting(event, db, () => {
                    signup.getHash(event.body.password, hash => {
...
const status = {
    checkOwnership: (event, client, success, issue) => {
        client.db(mongo.db).collection(mongo.user).findOne({
    ...
    lambda: (event, context, callback) => {
        ...
        if (event.body && event.body.channel) {
            mongo.connect((db, client) => {
                ...
                status.checkExisting(event, client, doc => {
