...
const mongo = {
    ...
    connect: (onConnect, issueCallback, collection) => {
        MongoClient.connect(process.env.MONGODB_URI, {
            ...
            if (client) {
                onConnect(client.db(mongo.db)
                    .collection(collection ? collection : 'users'), client)
            } else {
                issueCallback(connectionError)
            }
        })
    },
    newOid: () => { return new ObjectID() },
    ...
    removeFromPool: (providedClient) => {
        if (providedClient) {
            return connectionId => {
                providedClient.db(process.env.DB_NAME).collection(mongo.pool)
                    .deleteOne({
                        wss: connectionId
                    }, (deleteError, result) => {
