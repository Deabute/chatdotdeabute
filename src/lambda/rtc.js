...
var rtc = {
    ...
    offer: function (event, context, callback) { // TODO add last match behaviour
        ...
        if (event.body && event.body.sdp && event.body.oid) {
            ...
            mongo.connect(function onConnect(db, client) {
...
var prompt = {
    recordAnswer: function (client, event, success, issue) {
        if (event.body.answer) { // not passed for initial connection
...
var pool = {
    ...
    notifyAddition: function (db, event, success, issue) {
        var poolSize = 0;
        var free = 0;
        var duds = [];
        var docCount = 0;
        ...
        function finalFunc() { // can be called before or after steam end depending on which callback gets called last
            if (streamFinish && docCount === docFinished) {
                if (duds.length) { // if duds are removed in stream it will destroy topology (lose correct index in array by removing an element)
        ...
        function finish() { // note to async streaming callbacks that total stream is finished
            streamFinish = true;
            finalFunc();
        }
                ...
                mongo.connect(function onConnect(db, client) {
                    ...
                    function addToPool() {
                        pool.update(db, event, function onUpdate() {
                            pool.notifyAddition(db, event, function notified(free, poolSize) {
                                var resType = 'setPool';
