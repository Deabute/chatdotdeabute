...
const rtc = {
        ...
        return () => {
            if (matches.length) {
                ...
                socket.send(event, match.wss, {
                    ...
                    () => {
                        rtc.updateConnections(db, event, match.oid, msg => {
                            socket.response(event, {
                                action: 'nomatch'
    ...
    offer: (event, context, callback) => {
        ...
        catch (parseError) { socket.cb(callback, parseError) }
...
const pool = {
    channel: data => { // todo deprecate in favor of figuring this server side
        ...
        if (event.body) {
            ...
            if (event.body.oid) {
                mongo.connect((db, client) => {
                    ...
                    event.clientd = client
                    const addToPool = () => {
                        pool.update(db, event, () => {
                            pool.notifyAddition(db, event, (free, poolSize) => {
                                let resType = 'setPool'
                                if (free % 2 === 0) {
                                    resType = 'makeOffer'
                                ...
                                socket.response(event, { action: resType, pool: poolSize },
                                    ...
                                    () => {
                                        prompt.recordAnswer(client, event, socket.success(callback, client), issue)
    ...
    notifyReduce: (client, event, success, issue) => {
        ...
        mongo.stream(client.db(mongo.db)
            .collection(mongo.pool)
            .find({ channel: event.body.channel }), console.log, connection => {
                docCount++
    ...
    reduce: (event, context, callback) => {
        ...
        if (event.body && event.body.oid) {
            mongo.connect((db, client) => {
                event.clientd = client
                const issue = socket.issue(callback, client)
                pool.updateReduce(client, event, () => {
                    pool.notifyReduce(client,
                        event,
                        socket.success(callback, client),
                        issue
                    )
                }, issue)
            }, () => { socket.cb(callback, 'db connection issue') }, mongo.pool)
        } else {
            socket.cb(callback, 'validation issue')
        }
    }
